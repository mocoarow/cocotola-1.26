...
func Test_ActiveUserListRepository_Save_shouldInsertEntries_whenListIsNotEmpty(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "activeuser-save-org")
	userIDs := setupUsers(ctx, t, tx, orgID, "activeuser-save-org", 2)
...
func Test_ActiveUserListRepository_FindByOrganizationID_shouldReturnList_whenEntriesExist(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "activeuser-find-org")
	userIDs := setupUsers(ctx, t, tx, orgID, "activeuser-find-org", 3)
	repo := gateway.NewActiveUserListRepository(tx)
	list, err := domain.NewActiveUserList(orgID, userIDs)
	require.NoError(t, err)
...
func Test_ActiveUserListRepository_FindByOrganizationID_shouldReturnEmptyList_whenNoEntries(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "activeuser-empty-org")
	repo := gateway.NewActiveUserListRepository(tx)
...
func Test_ActiveUserListRepository_Save_shouldReplaceEntries_whenCalledTwice(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "activeuser-replace-org")
	userIDs := setupUsers(ctx, t, tx, orgID, "activeuser-replace-org", 3)
	repo := gateway.NewActiveUserListRepository(tx)
