...
// Check verifies database connectivity by querying the healthcheck1 table.
func (r *HealthRepository) Check(ctx context.Context) error {
	var result int
	if err := r.db.WithContext(ctx).Raw("SELECT 1 FROM healthcheck1 LIMIT 1").Scan(&result).Error; err != nil {
