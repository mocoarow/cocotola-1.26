...
func Test_GroupUsersRepository_Save_shouldInsertEntries_whenListIsNotEmpty(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "groupusers-save-org")
...
func Test_GroupUsersRepository_FindByGroupID_shouldReturnGroupUsers_whenEntriesExist(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "groupusers-find-org")
...
func Test_GroupUsersRepository_FindByGroupID_shouldReturnEmptyGroupUsers_whenNoEntries(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "groupusers-empty-org")
	groupIDs := setupGroups(ctx, t, tx, orgID, "groupusers-empty-org", 1)
	repo := gateway.NewGroupUsersRepository(tx)
...
func Test_GroupUsersRepository_Save_shouldReplaceEntries_whenCalledTwice(t *testing.T) {
	...
	tx := testDB.Begin()
	defer tx.Rollback()
	orgID := setupOrganization(ctx, t, tx, "groupusers-replace-org")
	userIDs := setupUsers(ctx, t, tx, orgID, "groupusers-replace-org", 3)
