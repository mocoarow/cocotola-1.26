...
func handleActiveListEvent[T any, L activeList[T]](
	...
	if err != nil {
		return fmt.Errorf("find organization %d: %w", organizationID, err)
	}
	...
	logger.InfoContext(ctx, fmt.Sprintf("added %s to active %s list", entityLabel, entityLabel),
		slog.Int(entityLabel+"_id", entityID),
		slog.Int("organization_id", organizationID))
