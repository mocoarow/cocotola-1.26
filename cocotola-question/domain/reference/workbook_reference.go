...
func NewWorkbookReference(id string, userID int, workbookID string, addedAt time.Time) (*WorkbookReference, error) {
	m := &WorkbookReference{
		id:         id,
		userID:     userID,
...
func ReconstructWorkbookReference(id string, userID int, workbookID string, addedAt time.Time) *WorkbookReference {
...
func (r *WorkbookReference) validate() error {
	...
	if r.userID <= 0 {
		return errors.New("workbook reference user id must be positive")
	}
	if r.workbookID == "" {
...
func (r *WorkbookReference) UserID() int { return r.userID }
